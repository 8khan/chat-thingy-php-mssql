...
public function chatHeartbeat() {	
	...
	$query = $this->database->queryselect($sql,$this->params1,$this->cursor1);	
	$items = '';
	$chatBoxes = array();
	while ($chatt = sqlsrv_fetch_Object($query)) {
		if (!isset($_SESSION['openChatBoxes'][$chatt->msgfrom]) && isset($_SESSION['chatHistory'][$chatt->msgfrom])) {
			$items = $_SESSION['chatHistory'][$chatt->msgfrom];
		}
		$chatt->message = $this->sanitize($chatt->message);
		$items .= <<<EOD
					   {
			"s": "0",
			"f": "{$chatt->msgfrom}",
...
EOD;
	...
	if (!isset($_SESSION['chatHistory'][$chatt->msgfrom])) {
		$_SESSION['chatHistory'][$chatt->msgfrom] = '';
	}
	$_SESSION['chatHistory'][$chatt->msgfrom] .= <<<EOD
						   {
			"s": "0",
			"f": "{$chatt->msgfrom}",
			"m": "{$chatt->message}"
...
EOD;
		...
		unset($_SESSION['tsChatBoxes'][$chatt->msgfrom]);
		$_SESSION['openChatBoxes'][$chatt->msgfrom] = date("Y-m-d h:i:s",strtotime($chatt->sent)); //2014-07-08 09:24:01
	}
...
EOD;
	...
	$sql = "insert into chat (msgfrom,msgto,message) values ('$from', '$to','$message')";
	$query = $this->database->query($sql);
	echo "1";	
	exit(0);
}
